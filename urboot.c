...
sudo make MCU=atmega128 WDTO=1S F_CPU=14745600L BAUD_RATE=115200 UARTNUM=0 RX=AtmelPE0 TX=AtmelPE1 VBL=1 EEPROM=0 CHIP_ERASE=0 URPROTOCOL=1 BLINK=1 AUTOFRILLS=0,6,4,3,2 NAME=urboot_m128_1s LED=AtmelPG4 LEDPOLARITY=1 SWIO=0
...
PORTE |= (1<<7);
/*
UR_DDR(AtmelPE7) &= ~UR_BV(AtmelPE7);
UR_PORT(AtmelPE7) &= ~UR_BV(AtmelPE7);

UR_DDR(AtmelPG4) |= UR_BV(AtmelPG4); 
UR_PORT(AtmelPG4) |= UR_BV(AtmelPG4); */
//PORTG |= (1<<4);
...
//_delay_us(100);
  ...
  while(!(UCSRnA & _BV(A_UDREn)))
   ...
   // _delay_us(100);
  UDRn = chr;
  while ( !( UCSRnA & (1<<A_TXCn)) );//Добавлено
	UCSRnA |= (1<<A_TXCn);//Добавлено
    PORTG &= ~(1<<0);
#elif UR_UARTTYPE == UR_UARTTYPE_LIN
